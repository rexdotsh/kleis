...
function renderAccounts() {
  ...
  $("#accounts-list").innerHTML = accounts
    .map((a) => {
      ...
      return `<div class="card" data-account-id="${a.id}"><div class="card-header"><div class="card-identity"><span class="status-dot ${s.class}"></span><span class="badge badge-${a.provider}">${a.provider}</span><span class="card-label">${escapeHtml(a.label || a.accountId || a.id)}</span>${a.isPrimary ? '<span class="badge badge-primary">primary</span>' : ""}</div><div class="card-actions">${a.isPrimary ? "" : `<button class="btn btn-ghost btn-sm" onclick="setPrimary('${a.id}')" type="button">set primary</button>`}<button class="btn btn-ghost btn-sm" onclick="refreshAccount('${a.id}')" type="button">refresh</button></div></div><div class="card-meta">${meta("status", `<span class="badge badge-${s.class}">${s.label}</span>`)}${meta("expires", a.expiresAt ? relativeTime(a.expiresAt) : "n/a")}${meta("last refresh", relativeTime(a.lastRefreshAt))}${a.lastRefreshStatus ? meta("refresh result", escapeHtml(a.lastRefreshStatus)) : ""}${meta("added", relativeTime(a.createdAt))}</div></div>`;
    })
    .join("");
}
    ...
    .map((k) => {
      ...
      const models = k.modelScopes
        ? k.modelScopes
            ...
            .join(", ")
        : '<span style="color:var(--text-tertiary)">all</span>';
      return `<div class="card"><div class="card-header"><div class="card-identity"><span class="card-label">${escapeHtml(k.label || "untitled")}</span><span class="badge badge-${badge}">${badge}</span></div><div class="card-actions">${revoked ? "" : `<button class="btn btn-danger btn-sm" onclick="revokeKey('${k.id}')" type="button">revoke</button>`}</div></div><div class="card-meta">${meta("key", `<span style="font-size:10px">${escapeHtml(maskKey(k.key))}</span>`)}${meta("providers", scopes)}${meta("models", models)}${meta("created", relativeTime(k.createdAt))}${k.expiresAt ? meta("expires", relativeTime(k.expiresAt)) : ""}</div></div>`;
    })
    .join("");
}
...
async function createKey() {
  ...
  try {
    ...
    reveal.innerHTML = `<span class="key-reveal-value">${escapeHtml(data.key.key)}</span><button class="btn btn-ghost btn-sm" onclick="copyToClipboard('${escapeHtml(data.key.key)}',this)" type="button">copy</button>`;
